...
class ChuNha extends Component {
    constructor(props) {
        super(props)
    ...
    componentDidMount = async () => {
        ...
        if (user && user.username) {
            if (Number(user.permission) === 3) {
                await this.layDanhSachChuNha(0, 10000, 3)
            }else {
    ...
    layDanhSachChuNha = async (indexPage, sodong, tinhtrang) => {
        let [err, result] = await to(fetchData.layDanhSachChuNha(indexPage, sodong, tinhtrang))

        if (err) {
            notification.open({ message: "Lỗi lấy danh sách chủ nhà!!!", description: `${err}` })
            return
        }
    ...
    render() {
        let { data, selectedRowKeys, isModalXacNhan, isModalHuy } = this.state
        const rowSelection = {
            selectedRowKeys,
        ...
        const columns = [
            {
                ...
                dataIndex: 'tenCN',
                key: 'tenCN',
                ...this.getColumnSearchProps('tenCN'),
            },
            {
                title: 'Địa chỉ',
                dataIndex: 'dChi',
...
export default withRouter(ChuNha)